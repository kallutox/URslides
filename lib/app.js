    ...
    form.on('file', function (name, file) {
        ...
        fs.rename(oldPath, newPath, function (err) {
            if (err) {
                ...
                let commentLit = {comments: currentSlide.comments};
                //create a new comment, which is the added audio comment
                commentLit.comments.push({_id: 0, _page: 1, _type: "audio", _content: sendPath});
                res.render("reader" , {
                    path: currentSlide.pdf,
                    name: currentSlide.name,
                    comments: JSON.stringify(commentLit),
                    edit: true,
                    idCount: currentSlide.idCount,
                });
            }
        });
    });
